...
美股收盘分析策略
================
扫描美股主要标的, 基于趋势+动量+量能+风险评分选出推荐

流程:
  1. get_us_stock_pool() — 获取标的池 (30+主要美股)
  2. _fetch_us_daily(symbol) — Yahoo Finance 拉日K (无需API Key)
...
数据源:
  Yahoo Finance 公开 API (yfinance), 无需 Key

调度:
  夜班期间 (22:30~08:30) 扫描一次, 美股收盘后分析
"""
...
try:
    from config import US_STOCK_PARAMS, TOP_N
except ImportError:
    US_STOCK_PARAMS = {
        "enabled": True,
        "top_n": 5,
...
def _guarded_yf_download(symbol, period="6mo", interval="1d"):
    """优先走 API 防护层拉 Yahoo Finance 数据"""
    try:
        import yfinance as yf
    except ImportError:
        print("[美股策略] yfinance 未安装, pip install yfinance")
        return None
    ...
    try:
        ...
        def _do_download():
            ...
            return df
        return guarded_call(_do_download, source="yfinance", retries=3)
    except ImportError:
        ticker = yf.Ticker(symbol)
...
def get_us_stock_pool():
    ...
    Returns:
        list[dict]: [{"symbol": "AAPL", "name": "苹果", "sector": "科技"}, ...]
    """
    print("[1/4] 获取美股标的池...")
    pool = []
    for symbol, info in US_STOCK_POOL.items():
        pool.append({
            "symbol": symbol,
            "name": info["name"],
            "sector": info["sector"],
        })
    print(f"  标的池: {len(pool)} 个美股")
    return pool


def _fetch_us_daily(symbol):
    """获取单个美股的日K线数据
    ...
    Returns:
        DataFrame or None (columns: open, high, low, close, volume)
    """
    try:
        df = _guarded_yf_download(symbol, period="6mo", interval="1d")
        if df is None or df.empty:
            return None
        # yfinance 返回的列名已经是 Open/High/Low/Close/Volume
...
def _analyze_us_stock(symbol, info, df):
    """分析单个美股, 返回评分结果或None"""
    ...
    # --- 评分 ---
    min_adx = US_STOCK_PARAMS.get("min_adx", 20)
    ...
    # s_volume
    min_vr = US_STOCK_PARAMS.get("min_volume_ratio", 1.2)
    vr_score = min(vol_ratio / 3.0, 1.0) if vol_ratio >= min_vr else vol_ratio / min_vr * 0.5
    s_volume = vr_score

    # s_risk: 波动率适中 (美股适中范围 0.15~0.40)
    ...
    if 0.15 <= volatility <= 0.40:
        ...
        vol_risk = max(0, 1.0 - (volatility - 0.40) / 0.4)
    s_risk = vol_risk

    # 加权总分
    weights = US_STOCK_PARAMS.get("weights", {})
    total_score = (
    ...
    if vol_ratio >= min_vr:
        reasons.append("量增" if vol_ratio >= 1.5 else "量平")
    if abs(pct_chg) >= 2:
        reasons.append(f"日涨跌{pct_chg:+.1f}%")
...
def run_us_stock_scan(top_n=None):
    """
    美股收盘分析主流程
    ...
    """
    if top_n is None:
        top_n = US_STOCK_PARAMS.get("top_n", 5)
    ...
    for i, item in enumerate(pool):
        symbol = item["symbol"]
        try:
            time.sleep(0.3)  # Yahoo Finance 限频
            df = _fetch_us_daily(symbol)
            if df is None or df.empty:
                fail_count += 1
                continue
            result = _analyze_us_stock(symbol, item, df)
    ...
    print(f"\n[4/4] 输出推荐")
    print("\n" + "=" * 65)
    print(f"  美股趋势推荐 TOP {len(selected)}  (共扫描{len(pool)}个标的)")
    print("=" * 65)

    for rank, item in enumerate(selected, 1):
        dir_icon = "▲" if item["direction"] == "long" else "▼"
        print(f"""
  {rank}. {item['symbol']} {item['name']} [{item['sector']}] {dir_icon} | 评分: {item['score']:.3f} | ${item['price']:.2f}
     信号: {item['reason']}
     技术: ADX={item['adx']:.0f}  RSI={item['rsi']:.0f}  MACD={item['macd']:.4f}  量比={item['vol_ratio']:.2f}
     涨跌: 日{item['pct_chg']:+.2f}%  5日{item['pct_5d']:+.2f}%  ATR=${item['atr']:.2f}""")

    return selected


def get_us_stock_recommendations(top_n=None):
    """标准化接口 (供 scheduler 调用)

    Returns:
        list[dict]: 标准化推荐列表
    """
    if not US_STOCK_PARAMS.get("enabled", True):
        print("[美股分析策略] 已禁用")
        return []

    if top_n is None:
        top_n = US_STOCK_PARAMS.get("top_n", TOP_N)

    try:
        selected = run_us_stock_scan(top_n=top_n)
...
if __name__ == "__main__":
    results = run_us_stock_scan()
    if results:
        try:
            from notifier import notify_wechat_raw
            lines = [f"美股收盘分析 TOP {len(results)}"]
            for r in results:
                d = "▲" if r["direction"] == "long" else "▼"
                lines.append(f"{d} {r['symbol']} {r['name']} ${r['price']:.2f} "
                             f"评分{r['score']:.3f} {r['pct_chg']:+.1f}%")
                lines.append(f"  {r['reason']}")
            notify_wechat_raw("美股收盘分析", "\n".join(lines))
            print("\n[微信推送完成]")
        except Exception as e:
            print(f"\n[微信推送失败] {e}")
